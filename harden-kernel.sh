...
pacman -S --needed --noconfirm base-devel bc xmlto kmod inetutils libelf pahole cpio perl tar xz apparmor dnscrypt-proxy opendoas zsh wget

KERNEL_VERSION=$(curl -s https://www.kernel.org/finger_banner | grep "stable:" | head -1 | awk '{print $NF}')
...
if [ "$BOOTLOADER" = "grub" ]; then
    ...
    if grep -q "^options" "$ENTRY"; then
        sed -i "s|^options.*|options $HARDENED_PARAMS|" "$ENTRY"
    else
        echo "options $HARDENED_PARAMS" >> "$ENTRY"
    fi
